...
/**
 * Represents an action submitted by a client thread to be processed by the tick
 * thread.
 *
 * Why this exists: client threads run concurrently with the tick engine. If
 * they modified World state directly, you'd need locks everywhere. Instead,
 * client threads just drop actions into a thread-safe queue, and the tick
 * thread processes them one at a time at the start of each tick.
 *
 * This is the same pattern OSRS uses — all player inputs are queued and
 * processed at the top of each game tick, which is why actions in OSRS always
 * feel like they "snap" to tick boundaries.
 */
